...
无偏：在渲染计算过程中不采用加速的算法或技巧，每一道光线都是一视同仁的，在任何重要性方面上无偏
...
沿着某个方向看材质上某个点（A），若该点可见（很重要），那么从这个点发出的Trace（打到B）会更重要

光能从B流向A，重要性从A流向B

#### 重要性计算

一个点会接受各个方向的入射光，每个入射光带了的辐照度为$\Phi_{i}$

那么给你一束光，这个光来自某条光线的概率为
$$
p_{i} = \frac{\Phi_{i}}{\Phi_{总}} = \frac{\Phi_{i}}{\sum_{j}\Phi_{j}}
$$
