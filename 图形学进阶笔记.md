...
时至今日，Render Graph已经成为现代图形API必备的功能，[UE4](https://docs.unrealengine.com/4.26/en-US/ProgrammingAndScripting/Rendering/RenderDependencyGraph/)和[Unity SRP](https://docs.unity3d.com/Packages/com.unity.render-pipelines.core@10.2/manual/render-graph-system.html)都做了支持
...
如下图，下图是一个延迟渲染，黄色的是Pass，代表着渲染操作，蓝色的是Buffer，代表着Resource
...
- 创建`RenderGraph`，并传入`RenderGraphExecuteParams`初始化
  - `RenderGraphExecuteParams`内含上下文（context）和命令队列（commandBuffer）
- 创建资源
  - 如`TextureHandle `、`ComputeBufferHandle `、`RendererListHandle `（句柄是一种虚拟资源，之前学GC的时候谈过）
