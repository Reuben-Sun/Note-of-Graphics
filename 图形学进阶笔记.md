...
![Beer定律](./markdown图片/Beer定律.png)

内散射的直观现象是“糖粉效应”，即由于物体中间的折痕更容易发生内散射，于是会显得特别亮

<img src="./markdown图片/糖粉效应.png" alt="糖粉效应" style="zoom: 33%;" />

可以通过修改Beer定律实现内散射

![Beers](./markdown图片/Beers.png)
...
<img src="./markdown图片/单次散射.png" alt="单次散射" style="zoom: 67%;" />
...
<img src="./markdown图片/单次散射实现.png" alt="单次散射实现" style="zoom:50%;" />
...
<img src="./markdown图片/米氏散射.png" alt="米氏散射" style="zoom: 67%;" />
...
![PVS可见性](markdown图片/PVS可见性.png)

<img src="markdown图片/PVS1.png" alt="PVS1" style="zoom: 33%;" />

<img src="markdown图片/PVS2.png" alt="PVS2" style="zoom:33%;" />
...
<img src="markdown图片/测试.jpg" alt="测试" style="zoom: 67%;" />
...
<img src="markdown图片/漫反射示意图.jpeg" alt="漫反射示意图" style="zoom: 33%;" />
$$
...
![辐射学单位与光学单位](markdown图片/辐射学单位与光学单位.jpg)
...
<img src="markdown图片/光通量与波长.png" alt="光通量与波长" style="zoom:50%;" />
...
$$
<img src="markdown图片/光通强度.jpeg" alt="光通强度" style="zoom:33%;" />

光线照射物体表面的方向用立体角$\omega $表示

<img src="markdown图片/平面角与球面角.jpeg" alt="平面角与球面角" style="zoom:33%;" />
...
$$
<img src="markdown图片/球面微分面积.jpeg" alt="球面微分面积" style="zoom:33%;" />

<img src="markdown图片/球面微分面积2.jpeg" alt="球面微分面积2" style="zoom: 33%;" />
...
<img src="markdown图片/角度翻倍.png" alt="角度翻倍" style="zoom:50%;" />
$$
...
<img src="markdown图片/几何衰减.jpeg" alt="几何衰减" style="zoom:33%;" />
$$
...
<img src="markdown图片/SH与阶数.jpeg" alt="SH与阶数" style="zoom:50%;" />
...
<img src="markdown图片/2维1阶SH.png" alt="2维1阶SH" style="zoom:50%;" />

- 三阶
  - 左：$r = sin2\theta$
  - 右：$r = cos2\theta$

<img src="markdown图片/2维2阶SH.png" alt="2维2阶SH" style="zoom:50%;" />
...
![球谐函数](markdown图片/球谐函数.jpeg)

<img src="markdown图片/球谐函数前三阶.jpeg" alt="球谐函数前三阶"  />
...
我们在进行PBR着色时，需要计算间接光（漫反射），可以用法线采Cubemap（本质是辐照度图），可以用uv2采Lightmap，也可以用SH拟合

SH的作用就是用来拟合辐照度图（所以也是用法线进行采样）
...
<img src="markdown图片/MC求pi.png" alt="MC求pi" style="zoom:72%;" />
...
*需要先知道被积函数的先验知识，即大致形状*

被积函数函数值大的部分对积分结果的贡献比较大，所以增加对其大采样
...
<img src="markdown图片/拉丁超立方体算法.jpeg" alt="拉丁超立方体算法" style="zoom: 67%;" />
...
一些比较简单的采样，如均匀分布、高斯分布、Gamma分布实现已经很完善，但是对于一些比较复杂的采样，需要配合一些采样策略
...
*这东西不会真的用，但其思想指导了MCMC*

我们先设置一个辅助**建议分布函数**$q(z)$，和一个常数K，使得$kq(z)\geqslant p(z)$

*kq(z)*即蓝线

<img src="markdown图片/拒绝采样法.png" alt="拒绝采样法" style="zoom:72%;" />
...
<img src="markdown图片/状态转移方程.png" alt="状态转移方程" style="zoom:50%;" />

如果$P_{ij}(i,i+1)=0$，则无法通过$i$状态转移到$j$状态

##### 对马尔可夫链采样

<img src="markdown图片/MCMC采样2.png" alt="MCMC采样2" style="zoom:50%;" />
...
<img src="markdown图片/概率分布变换2.png" alt="概率分布变换2" style="zoom:50%;" />

而体积比是什么呢？**格拉姆行列式(Gram Determinant)的平方根**，它的第$i,j$个元素$\langle v_{i},v_{j}\rangle $是$v_{i}和v_{j}$的内积

<img src="markdown图片/格拉姆行列式.png" alt="格拉姆行列式" style="zoom:50%;" />

<img src="markdown图片/均匀概率分布表达式.png" alt="均匀概率分布表达式" style="zoom: 50%;" />
...
<img src="markdown图片/多边形积分的不变性.jpeg" alt="多边形积分的不变性" style="zoom:50%;" />

- b：统一缩放来模拟粗糙度
- c：非一致缩放来模拟各向异性
- d：副对角线错切变换来模拟偏斜度（skewness）

![LTSDs](markdown图片/LTSDs.png)
...
$$
<img src="markdown图片/截断余弦分布.png" alt="截断余弦分布" style="zoom:50%;" />
...
<img src="markdown图片/二次投影.jpeg" alt="二次投影" style="zoom: 67%;" />
